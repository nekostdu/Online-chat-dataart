...
describe('PresenceDot', () => {
    it('uses the online colour for "online"', () => {
        const { container } = render(_jsx(PresenceDot, { presence: "online" }));
        const dot = container.querySelector('span');
        expect(dot).not.toBeNull();
        expect(dot.className).toContain('bg-emerald-500');
    });
    it('uses the AFK colour for "afk"', () => {
        const { container } = render(_jsx(PresenceDot, { presence: "afk" }));
    ...
    it('falls back to the offline colour when presence is missing', () => {
        const { container } = render(_jsx(PresenceDot, { presence: undefined }));
